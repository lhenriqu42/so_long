...
/*   Updated: 2024/12/30 13:07:18 by lhenriqu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static size_t ft_check_format_fd(int fd, char c, va_list args)
{
    ...
    if (c == 'c')
        ...
        count += ft_putnbr_base_fd(fd, va_arg(args, int), DECIMAL_BASE);
    else if (c == 'u')
        count += ft_putnbr_base_fd(fd, va_arg(args, unsigned int), DECIMAL_BASE);
    else if (c == 'x')
